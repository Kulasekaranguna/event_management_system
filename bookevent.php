...
.username {
 ...
 /* Added styles for required labels */
    ...
    <div class="content">
        ...
        <div class="maincontent">
            ...
            <form id="bookingForm" method="post" class="eventbooking">
                <label for="category" class="required">Event Type:</label>
                ...
                <label for="place" class="required">Event Place:</label>
                <input type="text" id="place" name="place">

                <label for="guests" class="required">No Of Guests:</label>
                <input type="text" id="guests" name="guests" onchange="calculateCost()">
                <label for="date" class="required">Date:</label>
    <input type="date" id="date" name="date">
    <button type="button" id="checkDateBtn">Check Date</button>
    <p id="errorMessage" style="color: red;">
    <p id="clickMessage" style="color: blue; font-style: italic; margin:10px;">(Please click this button to check the date availability)</p>
   

                <h2>Equipment</h2>
                <label><input type="checkbox" value="8000" name="dj" onchange="calculateCost()"> Dj Services</label><br>
                <label><input type="checkbox" value="7000" name="stage" onchange="calculateCost()"> Stage
                    Setup</label><br>
                ...
                <label><input type="checkbox" name="breakfast" value="8000" onchange="calculateCost()">
                    Breakfast</label><br>
                <label><input type="checkbox" name="lunch" value="8000" onchange="calculateCost()"> Lunch</label><br>
                <label><input type="checkbox" name="tea" value="2000" onchange="calculateCost()"> Tea &
                    Snacks</label><br>
...
document.getElementById("checkDateBtn").addEventListener("click", function() {
    ...
    if (selectedDate.trim() === "") {
        document.getElementById("errorMessage").textContent = "Please select a date.";
        return;
    }

    // Perform AJAX request to check date availability
    var xhr = new XMLHttpRequest();
    xhr.open("POST", "check_date_availability.php", true);
    xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
    xhr.onreadystatechange = function() {
        if (xhr.readyState === 4 && xhr.status === 200) {
            var response = xhr.responseText;
            if (response === "available") {
                document.getElementById("errorMessage").textContent = "The selected date is open for booking! You're good to go ahead and confirm your booking";
                document.getElementById("payNowBtn").style.display = "block"; // Show Pay Now button
            } else {
                document.getElementById("errorMessage").textContent = "Apologies, but it seems this date has already been reserved. Please pick another date.";
